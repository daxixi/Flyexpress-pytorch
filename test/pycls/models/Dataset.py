...
class flyDataset(Dataset):
    def __init__(self,feed,path='../project3/train/',is_train=True):
        super().__init__()
        self.path=path
        ...
        for i in range(30):
            self.scalar[i]=(len(self.ds)-distribution[i])/distribution[i]#/10+1
        print(self.scalar)
    ...
    def __getitem__(self,index):
        ...
        weights=torch.mul(self.scalar-1,labels)+1
