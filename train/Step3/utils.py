...
class Meter(object):
  ...
  def get(self):
    ...
    predicts=copy.deepcopy(self.predicts)
    predicts=predicts.T
    for i in range(30):
  ...
  def get_fp(self):
    ...
    for i in range(30):
      ...
      #print(i,p,r)
def find_thres(targets,predicts):
  targets=targets.T
  predicts=predicts.T
  threshold=[]
  for i in range(30):
    ...
    thres=0
    while thres<1:
        temp_p=np.where(p>thres,1,0)
        f1=f1_score(t,temp_p,average='binary')
        if f1>best:
            thres_b=thres
            best=f1
        thres+=0.01
